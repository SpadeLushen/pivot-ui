...
- The default Skill Library is `~/.pivot-ui/lib/skills`. An existing explicit library path in pi's Skill Pack configuration is left unchanged. If `libraryRoot` is relative, it is resolved relative to the directory containing `skill-packs.json`; the relative value itself is preserved in the configuration.
