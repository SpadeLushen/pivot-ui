...
test("resolveLibraryRoot anchors relative roots to the config directory without rewriting them", () => {
  const root = mkdtempSync(join(tmpdir(), "pivot-ui-packs-"));
  try {
    const path = join(root, "nested", "skill-packs.json");
    const config = { version: 2, libraryRoot: "../library", packs: [] };
    const expected = resolve(dirname(path), "../library");
    assert.notEqual(resolve(process.cwd(), "../library"), expected);
    assert.equal(resolveLibraryRoot(config, { configPath: path }), expected);
    assert.equal(config.libraryRoot, "../library");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test("resolveLibraryRoot leaves absolute roots unchanged", () => {
  const config = { version: 2, libraryRoot: "/home/me/skills", packs: [] };
  assert.equal(resolveLibraryRoot(config, { configPath: "/tmp/skill-packs.json" }), "/home/me/skills");
});
