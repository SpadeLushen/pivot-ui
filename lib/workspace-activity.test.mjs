...
test("running activity takes precedence over errors; errors reappear after completion", () => {
  assert.deepEqual(
    getWorkspaceActivity("/repo", sessions, new Set(["main"]), new Set(["worktree"]), new Set(["worktree"])),
    { isRunning: true, hasUnread: false },
  );
  assert.deepEqual(
    getWorkspaceActivity("/repo", sessions, new Set(), new Set(["worktree"]), new Set(["worktree"])),
