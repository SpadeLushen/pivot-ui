...
test("tracks only user message timestamps", () => {
  const entries = [
    userEntry("u1", null, "request", "2026-01-01T00:00:00.000Z"),
    assistantEntry("a1", "u1", "answer", "2026-01-03T00:00:00.000Z"),
  ];

  assert.equal(getLastUserMessageAt(entries), "2026-01-01T00:00:00.000Z");
});
...
test("keeps raw history visible when Pi edits only the model context", () => {
  const entries = [
    userEntry("u1", null, "original request"),
    { type: "context_edit", id: "edit", parentId: "u1", timestamp: "2026-01-01T00:00:01.000Z", targetId: "u1", replacement: null },
    assistantEntry("a1", "edit", "answer"),
  ];
  const context = buildSessionContext(entries);
  assert.deepEqual(context.entryIds, ["u1", "a1"]);
  assert.equal(context.messages[0].content, "original request");
});
...
test("defers historical thinking without changing live-session content", () => {
  ...
  assert.deepEqual(deferred.messages[1].content[0], {
    type: "thinking",
    thinking: "",
    thinkingPreview: "second line",
    deferred: true,
  });

  const full = buildSessionContext(entries);
  assert.equal(full.messages[1].content[0].thinking, "first line\nsecond line\n");
});
